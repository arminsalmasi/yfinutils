...
class Ticker:
    ...
    def history(
        ...
        if actions:
            ...
            # Map dividends
            if dividends:
                # ⚡ Bolt: Vectorize dividends mapping to avoid significant performance overhead
                div_values = list(dividends.values())
                div_dates = pd.to_datetime([item["date"] for item in div_values], unit="s").tz_localize("UTC").tz_convert(tz_name)
                div_amounts = [float(item["amount"]) for item in div_values]

                positions = df.index.get_indexer(div_dates, method="nearest")
            ...
            # Map stock splits
            if splits:
                # ⚡ Bolt: Vectorize stock splits mapping to avoid significant performance overhead
                split_values = list(splits.values())
                split_dates = pd.to_datetime([item["date"] for item in split_values], unit="s").tz_localize("UTC").tz_convert(tz_name)
                split_ratios = [float(item.get("numerator", 1)) / float(item.get("denominator", 1)) for item in split_values]

                positions = df.index.get_indexer(split_dates, method="nearest")
                valid_mask = positions != -1
