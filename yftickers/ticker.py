...
class Ticker:
    ...
    def history(
        ...
        if actions:
            ...
            # Map dividends (Vectorized operation to avoid looping overhead)
            if dividends:
                div_list = list(dividends.values())
                div_dates = [item["date"] for item in div_list]
                div_amounts = [float(item["amount"]) for item in div_list]
            ...
            # Map stock splits (Vectorized operation to avoid looping overhead)
            if splits:
                split_list = list(splits.values())
                split_dates = [item["date"] for item in split_list]
                split_amounts = [float(item.get("numerator", 1)) / float(item.get("denominator", 1)) for item in split_list]
                ...
                valid_mask = positions != -1
                valid_positions = positions[valid_mask]
                valid_amounts = np.array(split_amounts)[valid_mask]

                if len(valid_positions) > 0:
                    df.iloc[valid_positions, df.columns.get_loc("Stock Splits")] = valid_amounts
