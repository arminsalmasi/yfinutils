...
class Ticker:
    ...
    def history(
        ...
        if actions:
            ...
            # Map dividends (Vectorized optimization for faster dataframe insertion)
            if dividends:
                ...
                # Find closest index positions in one vectorized call
            ...
            # Map stock splits (Vectorized optimization)
            if splits:
                split_items = list(splits.values())
                split_dates = [item["date"] for item in split_items]
                split_ratios = [float(item.get("numerator", 1)) / float(item.get("denominator", 1)) for item in split_items]
