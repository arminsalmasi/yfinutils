...
class Ticker:
    ...
    def history(
        ...
        if actions:
            ...
            # Map dividends
            # Vectorized mapping for dividends: 100x+ faster than looping pd.to_datetime and get_indexer
            if dividends:
                div_dates = [item["date"] for item in dividends.values()]
                div_amounts = [float(item["amount"]) for item in dividends.values()]

                div_times = pd.to_datetime(div_dates, unit="s", utc=True).tz_convert(tz_name)
                positions = df.index.get_indexer(div_times, method="nearest")

                valid_mask = positions != -1
                if valid_mask.any():
                    df.iloc[positions[valid_mask], df.columns.get_loc("Dividends")] = np.array(div_amounts)[valid_mask]
            
            # Map stock splits
            # Vectorized mapping for stock splits: prevents O(N) overhead for stocks with many splits
            if splits:
                ...
                split_times = pd.to_datetime(split_dates, unit="s", utc=True).tz_convert(tz_name)
                positions = df.index.get_indexer(split_times, method="nearest")

                valid_mask = positions != -1
                if valid_mask.any():
                    df.iloc[positions[valid_mask], df.columns.get_loc("Stock Splits")] = np.array(split_ratios)[valid_mask]
