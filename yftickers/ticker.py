...
class Ticker:
    ...
    def history(
        ...
        if actions:
            ...
            # Map dividends
            if dividends:
                # OPTIMIZATION: Vectorize date parsing and assignment to avoid pandas overhead in loops
                div_dates = []
                div_amounts = []
                for div_item in dividends.values():
                    div_dates.append(div_item["date"])
                    div_amounts.append(float(div_item["amount"]))
