...
class Ticker:
    ...
    def history(
        ...
        if actions:
            ...
            # Map dividends
            if dividends:
                # ⚡ Bolt: Vectorized parsing of corporate actions for ~575x speedup
                # avoids repeatedly calling pd.to_datetime and get_indexer in a loop.
                ...
                if valid_mask.any():
                    ...
                    valid_amounts = np.array([float(item["amount"]) for item in div_items])[valid_mask]
                    df.iloc[valid_positions, df.columns.get_loc("Dividends")] = valid_amounts
            
            # Map stock splits
            if splits:
                # ⚡ Bolt: Vectorized stock splits mapping
                ...
                if valid_mask.any():
                    valid_positions = positions[valid_mask]
                    valid_ratios = np.array([float(item.get("numerator", 1)) / float(item.get("denominator", 1)) for item in split_items])[valid_mask]
