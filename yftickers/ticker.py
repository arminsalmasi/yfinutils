...
class Ticker:
    ...
    def history(
        ...
        if actions:
            ...
            # Map dividends (⚡ Bolt: Vectorized mapping of time-series events avoids Python loop overhead)
            if dividends:
            ...
            # Map stock splits (⚡ Bolt: Vectorized mapping of time-series events avoids Python loop overhead)
            if splits:
                split_items = list(splits.values())
                split_dates = [item["date"] for item in split_items]
                split_amounts = [float(item.get("numerator", 1)) / float(item.get("denominator", 1)) for item in split_items]
