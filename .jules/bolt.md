...
## 2024-05-18 - Vectorized Pandas Event Mapping
**Learning:** When mapping time-series events (like corporate actions) to a Pandas DataFrame, using vectorized operations (e.g., passing a list of dates to `pd.to_datetime` followed by `get_indexer`) instead of iterating through events in a Python loop avoids significant performance overhead.
**Action:** Always prefer vectorized Pandas operations for mapping and aligning time-series arrays over Python `for` loops.
