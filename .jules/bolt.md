...
## 2025-02-19 - Vectorize Corporate Actions Mapping
**Learning:** When mapping time-series events (like corporate actions) to a Pandas DataFrame, iterating through events in a Python loop to match indices using `get_indexer` incurs significant performance overhead.
**Action:** Use vectorized operations by passing a list of dates to `pd.to_datetime` followed by `get_indexer` on the DataFrame's index. This reduces processing time dramatically.
